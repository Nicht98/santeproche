...
export const providerRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/providers/facility', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    const [facility] = await db.execute(
      ...
      [body.name, body.kind, body.address, body.lng, body.lat, body.phone, JSON.stringify(body.openingHours || []), request.user!.id]
    );
  ...
  fastify.post('/facilities/:id/stock', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    const [stock] = await db.execute(
      `INSERT INTO facility_stock (facility_id, drug_id, status, quantity, notes, updated_by, updated_at)
       ...
       RETURNING *`,
      [id, drugId, status, quantity, notes, request.user!.id]
    );
