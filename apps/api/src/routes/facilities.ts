...
export const facilityRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/facilities', async (request, reply) => {
    const { lat, lng, radius_km, kind, open_now: _openNow, limit = '20', offset = '0' } = request.query as Record<string, string>;
  ...
  fastify.get('/facilities/:id', async (request, reply) => {
    ...
    const [facility] = await query('SELECT * FROM facilities WHERE id = $1 LIMIT 1', [id]);
